        ...
        <>
            <div className="grid-wrapper">
                ...
                <ul ref={shopWrapper} className={`grid-container${props.grid ? ` ${props.grid}` : ""}${loading ? " loading" : ""}${props.className ? ` ${props.className}` : ""}${props.filter === false ? "" : " mt-28 md:mt-[4.5rem] sm:mt-8"}`}>
                    ...
                    {
                        (props.data || shopWideData).map((item, i) => {
                            return (
                                <li className={`grid-item${item.double_col ? " grid-item-double" : ""} ${item.category.map(item => item.split(" ").join("")).toString().split(",").join(" ").toLowerCase()}`} key={i}>
                                    <m.div
                                        ...
                                        <div className="product-image relative overflow-hidden rounded-md">
                                            <Link aria-label="link" href="/shop/single-product/">
                                            ...
                                            <div className="product-hover-bottom absolute bottom-0 w-full left-0 transition-default py-[35px] text-center">
                                                <OverlayTrigger overlay={(props) => (<Tooltip className="text-xxs font-serif" {...props}>ADD TO CART</Tooltip>)} placement="top"><Link aria-label="shopwide" href="#" rel="noreferrer" className="product-link-icon move-top-bottom relative rounded-full bg-[#fff] text-darkgray inline-block text-center transition-default m-[5px] h-[40px] w-[40px] leading-[42px]">
                                                    <i className="feather-shopping-cart m-0 text-base "></i>
                                                </Link></OverlayTrigger>
                                                <OverlayTrigger overlay={(props) => (<Tooltip {...props}>QUICK SHOP</Tooltip>)} placement="top"><Link aria-label="shopwide" href="#" rel="noreferrer" className="product-link-icon move-top-bottom relative rounded-full bg-[#fff] text-darkgray inline-block text-center transition-default m-[5px] h-[40px] w-[40px] leading-[42px]">
                                                    <i className="feather-zoom-in m-0 text-base"></i>
                                                </Link></OverlayTrigger>
                                                <OverlayTrigger overlay={(props) => (<Tooltip {...props}>ADD TO WISHLIST</Tooltip>)} placement="top"><Link aria-label="shopwide" href="#" rel="noreferrer" className="product-link-icon move-top-bottom relative rounded-full bg-[#fff] text-darkgray inline-block text-center transition-default m-[5px] h-[40px] w-[40px] leading-[42px]">
                                        ...
                                        <div className="product-footer text-center pt-[25px] xs:pt-[10px]">
                                            <Link aria-label="link" href="/shop/single-product" className="text-darkgray hover:text-black font-medium inline-block">{item.title}</Link>
                                            {
